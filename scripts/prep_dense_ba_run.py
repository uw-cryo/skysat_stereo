...
def main():
    ...
    print(f'Found {len(triplet_stereo_matches)} dense matches')
    ...
    for idx,match in tqdm(triplet_stereo_matches):
    ...
    if modify_overlap == 1:
        ...
        print(f"Saving adjusted overlap list at {out_overlap}")
        with open(out_overlap,'w') as foo:
            for idx,img1 in enumerate(new_img1):
                out_str = f'{img1} {new_img2[idx]}\n'
                f.write(out_str)
